...
var UIManager = /** @class */ (function (_super) {
    ...
    UIManager.prototype.tryShowUI = function (uiScript, uiResPaths, caller, callBack, parentNode) {
        ...
        if (uiUnit.uiState == UIState.Hide) {
            ...
            if (uiUnit.uiResPaths.length > 0) {
                ...
                }), Laya.Handler.create(this, this.updateProgress, null, false));
            }
            else {
                this.showPanel(uiScript, uiUnit, parentNode);
                if (caller && callBack)
    ...
    UIManager.prototype.addLoadingMask = function () {
        var uiUnit = this.getUIUnit(common.panel.LoadingMask.toString());
        uiUnit.uiState = UIState.Show;
        uiUnit.uiResPaths = common.panel.LoadingMask.assets;
        this.showPanel(common.panel.LoadingMask, uiUnit, this.maskNode);
    };
    UIManager.prototype.ShowLoadingMask = function () {
        common.panel.LoadingMask.Show();
    };
    UIManager.prototype.updateProgress = function (num) {
        //if (GameMain.DEBUG) console.log("UIManager ShowLoadingMask===========>", num);
        var ui = this.GetUI(common.panel.LoadingMask);
        if (ui) {
            //ui.showLabel(true);
            ui.onProgress(num);
        }
    };
    UIManager.prototype.HideLoadingMask = function () {
