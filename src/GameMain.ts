...
class GameMain {
    ...
    public static VERSION: string = "03.11.28.1500";
    ...
    protected getParams(): boolean {
       ...
       // paramsJson.openDebug = (paramsJson.openDebug == null || paramsJson.openDebug == undefined) ? true : paramsJson.openDebug;
        ...
        if (wsUrl == undefined || wsUrl == "") {
            ...
            if (GameMain.DEBUG) console.log("参数没有wsUrl");
            return false;
    ...
    protected loaded(): void {
        ...
        window.document.addEventListener("message", function (event) {
            ...
            if (action == "lifecycle") {
                if (GameMain.DEBUG) console.log("GameMain lifecycle111111=============>", data);
                if (data) {
                    if (GameMain.DEBUG) console.log("GameMain lifecycle=============kaiqi>", event);
                    //SoundManager.PlayBGM();
                    Laya.SoundManager.muted = false;
                }
                else {
                    if (GameMain.DEBUG) console.log("GameMain lifecycle=============close>");
